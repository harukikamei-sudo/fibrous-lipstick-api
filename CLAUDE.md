...
├── ui_app.py          ★Streamlit UI(Lv2): 唇プリセット→/recommend→TOP5を唇画像合成表示
│                      合成= L比率ブレンド(質感保持)+a,b置換。assets/lips/ or ダミー生成
│                      `pip install -r requirements-ui.txt && streamlit run ui_app.py`
│                      API 本体とは別依存(requirements-ui.txt)。HF には載せない(ローカル用)
├── assets/lips/       唇プリセット画像(lip_<preset>.png, α=唇マスク)。無ければダミー生成
...
| UI-Lv2 | Streamlit ui_app.py(唇画像に塗布シミュ合成。L比率ブレンド+a,b置換) | ✅ 完了 (合成・headless確認)。唇画像はassets/lips or ダミー生成。実画像が来れば差し替え |
