# KAWANO_HANDOFF.md — Kawano さん向けハンドオフ

> Fibrous Lipstick AR アプリのハルキ × Kawano 連携をスムーズに始めるための、
...
> **Kawano は AR とユーザー UI、ハルキは色の物理計算と推薦ロジック。**
> **HTTP API 4 つで握手する。**
> **ハルキ側は実装済み・本番稼働中、Kawano 側はこれから。**
...
│                                                                              │
│  📷 肌を自撮り    →  PC判定 (例: ブルベ夏)        ┐                          │
│  👄 ノーリップ唇   →  唇 Lab = (62, 22, 12)       │  ★ ぜんぶ Kawano AR が担当 │
│  🎴 ペア10問選ぶ   →  「どっち好き?」を10回        ┘                          │
│                                                                              │
│  ↓ 結果をハルキ API に送信                                                   │
│                                                                              │
│  ☁ ハルキ API: ベイズ事前分布を構築 → ミナ専用の UserState を返す             │
└──────────────────────────────────────────────────────────────────────────────┘
...
│  ↓ 観測ログをハルキ API に送信                                                │
│                                                                              │
│  ☁ ハルキ API: ベイズ更新 → 「ミナの好み」を学習 → 更新後の UserState を返す  │
└──────────────────────────────────────────────────────────────────────────────┘
...
```
┌───────────────────────────────┬─────────────────────────────────────────┐
│  🎨 Kawano 担当(AR + UI)     │  ⚙ ハルキ担当(計算サーバー、実装済)    │
├───────────────────────────────┼─────────────────────────────────────────┤
...
### Kawano が「やらなくていい」こと
- **K-M 物理計算は不要**(ハルキの API が `effective_Lab` を返す → それを唇に合成するだけ)
- **ΔE2000 などの色差計算は不要**(ハルキ側で全部やる)
- **商品データは持たなくていい**(ハルキの API がカタログ持ってる)
- **ペア定義は持たなくていい**(`/v13/pair_compare/init` で取れる)
- **ベイズ更新の数学は知らなくていい**(API がブラックボックスで返す)

### Kawano が「やらないといけない」こと
- AR の絵作り(これが価値の中心)
- ユーザー UI(撮影誘導、ペア提示、AR スライダー、いいね/微妙)
- `UserState` の保存(GAS / Firebase / 自前 BE / なんでも可)
- ハルキ API への HTTP リクエスト(4 種類)
...
│         → Kawano は effective_Lab を唇に合成、AR で表示                   │
└─────────────────────────────────────────────────────────────────────────┘
```
...
## 5. UserState の中身(これを Kawano が保存・送受信する)

```jsonc
{
  "user_id": "mina_001",         // Kawano 側で発行
...
### Q2. 通信モデル
- 現状: 同期 REST + CORS 開放
- GAS 経由でも Kawano AR 直叩きでも対応可
- Webhook / SSE が必要なシーン ある?

### Q3. ペア比較 10 問の中身 ⭐ 重要
- 俺が `_PAIR_SPECS`(`pair_compare.py:158`)で仮に組んでる
- 商品の組み合わせ・提示順は Kawano UI のフロー次第で差し替えたい
...
- Kawano が AR で見せたい「印象タグ」と整合させたい
- ここを合わせると推薦精度が一気に上がる
...
- 規模が大きくなったら Kawano 側でテーブル保持する?
- 現状のままで OK ならノータッチ

### Q7. ユーザー識別 / 認証
- 今は `user_id` は Kawano が任意発行
- ハルキ側に最低限の users テーブル要る?
- アカウント機構は Kawano 側? OAuth?

---

## 8. 今すぐ何ができるか(Kawano が試せる手順)
...
- Kawano AR の技術スタック(Unity? Web AR? ネイティブ?)を教えてほしい
- 撮影 UI のスペック(画像サイズ・形式)が決まったら共有してほしい

### Kawano 側にお願いしたいこと(=ハルキが対応する)
- API のレスポンスに `image_url` を含めるか
- レスポンスのフィールド名を Kawano が使いやすい名前に変える
